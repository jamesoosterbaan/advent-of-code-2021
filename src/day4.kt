...
fun main(args: Array<String>) {
    ...
    val gameBoardsPart1: MutableList<GameBoard> = mutableListOf()
    val gameBoardsPart2: MutableList<GameBoard> = mutableListOf()

    // Set up the game boards - this is a bit hacky, but I didn't want to implement any deep copies, so I've just instantiated a separate list for
    // each puzzle part.
    for (index in 0 until (gameBoardInputs.size)/5) {
        val gameBoardPart1 = GameBoard(gameBoardInputs.slice(index*5 until index*5 + 5))
        val gameBoardPart2 = GameBoard(gameBoardInputs.slice(index*5 until index*5 + 5))
        gameBoardsPart1.add(gameBoardPart1)
        gameBoardsPart2.add(gameBoardPart2)
    }

    day4part1(draws, gameBoardsPart1)
    day4part2(draws, gameBoardsPart2)
}

fun day4part1(draws: List<Int>, gameBoards: List<GameBoard>) {
    println("\n--- Part 1 ---\n")
...
fun day4part2(draws: List<Int>, gameBoards: List<GameBoard>) {
    println("\n--- Part 2 ---\n")
 ...
 */
class GameBoard(initialState: List<List<Int>>) {
